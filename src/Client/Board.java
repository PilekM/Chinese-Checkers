...
public class Board {
    ...
    void markField(Field board[][],int rows)
    {
        for(int i=0;i<3*rows+1;i++)
        {
            for(int j=0;j<=i; j++)
            {
                board[3*rows-i+j*2][i*2].setField(true);
            }
        }
        int k=0;
        ...
        for(int i=4*rows;i>=rows;i--)
        {
            for(int j=0;j<=k; j++)
            {
                board[3*rows-k+2*j][i*2].setField(true);
            }
            k++;
        }
    }
    ...
    void upsideDown(Field board[][],int rows)
    {
        for(int i=0;i<rows;i++)
        {
            for(int j=0;j<=i;j++)
            {
                board[3*rows-i+2*j][i*2].setBase(1);
                board[rows-1-i+2*j][2*(2*rows+1+i)].setBase(3);
                board[5*rows+1-i+2*j][2*(2*rows+1+i)].setBase(5);
    ...
    void downsideUp(Field board[][],int rows)
    {
        for(int i=0;i<rows;i++)
        {
            for(int j=0;j<=i;j++)
            {
                board[3*rows-i+2*j][8*rows-i*2].setBase(6);
                board[5*rows+1-i+2*j][2*(2*rows-1-i)].setBase(4);
                board[rows-1-i+2*j][2*(2*rows-1-i)].setBase(2);
